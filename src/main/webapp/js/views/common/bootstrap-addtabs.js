...
window.Addtabs = {
    ...
    add: function (opts) {
        //debugger;
        var id = 'tab_' + opts.id;
        $(window.parent.document).find('li[role = "presentation"].active').removeClass('active');
        $(window.parent.document).find('div[role = "tabpanel"].active').removeClass('active');
        //如果TAB不存在，创建一个新的TAB
        if (!$(window.parent.document).find("#" + id)[0]) {
            //创建新TAB的title
        ...
        //激活TAB
        $(window.parent.document).find('#tab_' + id).addClass('active');
        $(window.parent.document).find('#' + id).addClass('active');
        Addtabs.drop();
    },
    pop: function (id,e) {
        $(window.parent.document).find('body').find('#popMenu').remove();
    ...
    close: function (id) {
        ...
        if (obj.find("li.active").attr('id') === "tab_" + id) {
            $(window.parent.document).find("#tab_" + id).prev().addClass('active');
            $(window.parent.document).find("#" + id).prev().addClass('active');
        }
        //关闭TAB
        $(window.parent.document).find("#tab_" + id).remove();
    ...
    closeAll: function () {
        ...
        $(window.parent.document).find('#'+firstID).addClass('active');
        Addtabs.drop();
    },
    drop: function () {
        ...
        //检测是否已增加
        if (!$(window.parent.document).find('.tabdrop').html()) {
