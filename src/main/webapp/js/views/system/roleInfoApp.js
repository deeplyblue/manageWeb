...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        vm.jobAllocate = jobAllocate;

        function resourceAllocate(id) {
            $http.post(config.ctx + '/system/role/initResource', $httpParamSerializerJQLike({id: id}))
                .then(function (response) {
                    OpenService({
                        modalTitle: '权限分配',
        ...
        function jobAllocate(id) {
            $http.post(config.ctx + '/system/role/initJobResponsibility', $httpParamSerializerJQLike({roleId: id}))
                .then(function (response) {
                    OpenService({
                        modalTitle: '岗位职责分配',
                        modalBody: 'toJobAllocate',
                        url: 'jobAllocate',
                        item: {
                            id: id,
                            jobData: response.data.object
                        }
                    }, function (item) {

                    }, 'privateJobAllocateModalInstanceCtrl');
                }, function (response) {
                    alert('访问失败');
                })
        }
...
angular.module('myApp').controller('privateJobAllocateModalInstanceCtrl',['$scope', '$uibModalInstance', 'modalItem', '$http','CheckboxService',
    '$httpParamSerializerJQLike', '$log', '$timeout', function ($scope, $uibModalInstance, modalItem, $http,CheckboxService,
                                                                         $httpParamSerializerJQLike, $log, $timeout) {
    ...
    var vm = $scope.vm = {
        ...
        vm.updateChecked = function updateChecked($event, id) {
            CheckboxService.updateChecked($event,id);
            console.info(id);
            $log.debug(id);
            $log.debug(CheckboxService.getChecked());
    ...
    $scope.save = function () {
       ...
       console.info( CheckboxService.getChecked());
        var ids= CheckboxService.getChecked();
        console.info(ids);
        $http.post(vm.url, $httpParamSerializerJQLike({
                id: vm.item.id,
                ids: ids
