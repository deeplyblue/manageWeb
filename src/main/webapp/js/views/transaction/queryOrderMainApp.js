...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        vm.queryOrderMainDetail = queryOrderMainDetail;
        ...
        function download() {
            ...
            vm.url =config.ctx +'/orderMain/download';
            ...
            $http.post(vm.url, $httpParamSerializerJQLike(vm.postData))
                .then(function (response) {
                    ...
                    if (data.success) {
                        if(!confirm("文件已经保存到下载中心，是否跳转到下载中心？")){
                            return;
                        }else{
                            window.location.href=config.ctx +'/download/userDownload/init';
                        }
        ...
        function queryOrderMainDetail(ourTransNo) {
            var results;
            $http.post(config.ctx + '/orderMain/queryOrderMainDetail', $httpParamSerializerJQLike({ourTransNo: ourTransNo}))
                .then(function (response) {
                    var data = response.data;
                    if (data.success) {
                        results = data.object;
                        OpenService({
                            ...
                            url: 'queryDetail',
                            item: {results: results, ourTransNo: ourTransNo},
                            cached: vm.cached
