...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        var queryBean = {
            beginOrderDate: DateCalculateService.getToday(),
            endOrderDate: DateCalculateService.getToday()
        }

        vm.queryBean = queryBean;

        vm.doRefund = doRefund;
        ...
        function doRefund(ourTransNo) {
            $http.post(config.ctx + '/mchntplatform/mchntRefund/refundApply', $httpParamSerializerJQLike({'ourTransNo': ourTransNo}))
                .then(function (response) {
                    var data = response.data;
                    if (data.success) {
                        alert("申请成功");
