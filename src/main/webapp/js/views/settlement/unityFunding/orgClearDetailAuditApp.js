...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        function toAuditDetail(handleBatchNo) {
            var pagination;
            $http.post(config.ctx + '/settlement/unityFunding/orgClearDetailAudit/queryAuditDetail',
                ...
                .then(function (response) {
                    ...
                    if (data.success) {
                        ...
                        OpenService({
                            ...
                            $log.debug(item);
                            vm.queryDetail();
                        }, 'OrgClearAuditDetailCtrl', 'myModalNoSave.html');
                    } else {
                        alert('未找到相关数据');
                        return false;
                    }
...
angular.module('myApp').controller('OrgClearAuditDetailCtrl',['$scope', '$uibModalInstance', 'modalItem', '$http',
    ...
    vm.audit = function (handleBatchNo, auditStatus) {
        if(confirm("确认审核？"))
        {
            $http.post(config.ctx + '/settlement/unityFunding/orgClearDetailAudit/audit',
                $httpParamSerializerJQLike({
                    handleBatchNo: handleBatchNo,
    ...
    vm.auditFail = function (handleBatchNo, clrDate,clrType) {
        if(confirm("确认审核？"))
        {
            vm.url = config.ctx + '/settlement/unityFunding/orgClearDetailAudit/auditFail';
            //组织参数：查询条件 + 分页数据
            var postData = {};
