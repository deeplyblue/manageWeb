...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        function addItem() {
            OpenService({
                modalTitle: '添加商户业务资金源关系 ',
                modalBody: 'toAdd',
                url: 'add',
                item: {},
                cached: {
                    MERCHANT_CODE: CacheService.getCache('MERCHANT_CODE'),
                    BUSINESS_FUNDING_SOURCES: vm.cached.BUSINESS_FUNDING_SOURCE
                }
            }, function () {
        ...
        function updateItemEnableFlag(item) {
            var enableTypeList = {'0':'1','1':'0'};
            var postData = angular.copy(item);
            postData.enableType = enableTypeList[item.enableType];
            $http.post(config.ctx+"/merchant/business/type/update", $httpParamSerializerJQLike(postData))
                .then(function (response) {
                    if (response.data.success) {
                        item.enableType = postData.enableType;
                        alert("操作成功");
                    } else {
        ...
        function updateItem() {
            ...
            }, "updateMerchantContactsModalInstanceCtrl");
        }
