...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        function resetForm() {
            //var companyCode = vm.queryBean.companyCode;
            var companyType = vm.queryBean.companyType;
            vm.queryBean = {};
            //vm.queryBean.companyCode=companyCode;
            vm.queryBean.companyType=companyType;
        ...
        function addItem() {
            $http.post(config.ctx + '/institution/transRight/initResource', $httpParamSerializerJQLike({}))
                .then(function (response) {
                    OpenService({
                        modalTitle: '增加交易权限',
                        modalBody: 'toAdd',
        ...
        function updateItem() {
            ...
            $log.info("----%s", temp);
            $http.post(config.ctx + '/institution/transRight/initResource', $httpParamSerializerJQLike({merchantCode: temp,companyType:vm.queryBean.companyType}))
                .then(function (response) {
                    OpenService({
                        modalTitle: '交易权限权限分配',
                        modalBody: 'toUpdate',
