...
angular.module('myApp')
    ...
    .controller('queryCtrl',['$scope', '$http', '$httpParamSerializerJQLike', '$log', 'OpenService', 'CheckboxService',
        ...
        function queryDetail() {
            ...
            $http.post(vm.url, postData,headConfig)
                .then(function (response) {
                    var data = response.data;
                    if (data.success) {
        ...
        function resetItem(obj){
            ...
            $http.post(config.ctx+"/bank/trans/down/delete", obj,headConfig)
                .then(function (response) {
                    var data = response.data;
                    if (data.success) {
                        vm.pagination.list.splice(vm.pagination.list.indexOf(obj), 1);
                        alert("操作成功")
                    } else {
                        alert(data.msg);
                    }
