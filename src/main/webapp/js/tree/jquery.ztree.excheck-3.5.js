...
(function($){
	...
	_zTreeTools = function(setting, zTreeTools) {
		...
		zTreeTools.setAllRefundNodes = function(checked) {
			view.repairMiaoAllChk(this.setting, !!checked);
		}
	...
	_view = {
		checkNodeRelation: function(setting, node) {
			...
			if (setting.check.chkStyle == r.STYLE) {
				...
				if (node[checkedKey] && (!node[childKey] || node[childKey].length==0 || setting.check.chkboxType.Y.indexOf("s") > -1)) {
					...
					//TODO   添加临时联动 选普通订单时 同时选中 商户退款 
					if($.fn.zTree.linkRefund)
					{
						//TODO 此功能暂时取消
						/*if(node.name.indexOf("普通订单")>-1){
							var arr;
							if(node.getParentNode()){
							arr = $.grep(node.getParentNode().children, function (a) { return a.name.indexOf('商户退款')>-1; });
							view.setSonNodeCheckBox(setting, arr[0], true);
							}
						}*/
					}
		...
		//TODO  新增方法全选 商户退款 
		repairMiaoAllChk: function(setting, checked) {
			if (setting.check.enable && setting.check.chkStyle === consts.checkbox.STYLE) {
				var checkedKey = setting.data.key.checked,
				childKey = setting.data.key.children,
				root = data.getRoot(setting);
				for (var i = 0, l = root[childKey].length; i<l ; i++) {
					var nodes = root[childKey][i];
					/*if (nodes.nocheck !== true) {
						nodes[checkedKey] = checked;
					}*/
					var checkedKey = setting.data.key.checked,
					childKey = setting.data.key.children;
					 
					for(var x = 0,y = nodes[childKey].length; x<y; x++){
						var node = nodes[childKey][x];
						/*if (node.nocheck !== true) {
							node[checkedKey] = checked;
						}*/
						
						if($.fn.zTree.linkRefund && (node.name == "商户退款")){
							
							 
							if(node.pId == "07" && !checked)
							{
								//内部渠道的 不取消
								continue;
							}
							view.setParentNodeCheckBox(setting, nodes, checked);
							view.setSonNodeCheckBox(setting, node, checked);
						}
					}
