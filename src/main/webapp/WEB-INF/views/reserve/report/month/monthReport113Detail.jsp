...
<div style="border: dashed grey 1px;padding:2px 0px 5px 0px;margin-left: inherit;margin-right: 5px;
    ...
    <table border="1" style="width: 1300px;text-align: center;word-wrap:break-word;word-break:break-all;">
        ...
        <tr>
            ...
            <td rowspan="2">期初备付金账户中押金余额</td>
            <td colspan="2">本期备付金账户中实际收到的押金额</td>
            <td colspan="2">本期备付金账户中实际付出的押金额</td>
            <td colspan="2">期末备付金账户中押金余额</td>
        </tr>
        <tr>
            <td>转账方式收到</td>
            <td>现金形式缴存</td>
            <td>转账方式直接付出</td>
            <td>现金形式先行赎回业务中申请结转金额</td>
        </tr>
        </thead>
        <tr ng-repeat="bean in vm.item.list track by $index">
            <%--<td><input name="check" type="checkbox" ng-model="bean._checked"></td>--%>
                ...
                <td>{{bean.n01/1000000}}</td>
                <td>{{bean.n02/1000000}}</td>
                <td>{{bean.n03/1000000}}</td>
                <td>{{bean.n04/1000000}}</td>
                <td>{{bean.n05/1000000}}</td>
                <td>{{bean.n06/1000000}}</td>
        </tr>
    </table>
