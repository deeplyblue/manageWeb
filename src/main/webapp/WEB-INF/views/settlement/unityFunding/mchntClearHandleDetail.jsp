...
<div common-modal-form>
    <div id="withExport">
        ...
        <shiro:hasPermission name="mchntClearDetail_down">
            <button onclick="$('#withExport .myTable').tableExport({type:'excel',escape:'false',ignoreColumn:[21],tableName:'商户周期经办'})">
                下载
            </button>
        </shiro:hasPermission>
        <table table-detail>
            ...
            <tr class="ignoreRow">
                ...
                <td>{{bean.adjustModel.payableAmt - bean.adjustModel.receivableAmt | currency:''}}</td>
                <td style="padding: 0 0 0 0"><input type="number" style="width: 45px"
                                                    ng-model="bean.adjustModel.reFeeAmt">
                </td>
                <td style="padding: 0 0 0 0"><input type="number" style="width: 45px" ng-model="bean.adjustModel.feeAmt">
                </td>
                <td>{{bean.adjustModel.reFeeAmt - bean.adjustModel.feeAmt | currency:''}}</td>
                ...
                <td>
                    <button ng-show="bean.clrFlag == '0'" ng-disabled="commonModalForm.$invalid"
                            ng-click="vm.saveAdjust(bean,bean.adjustModel)">保存
                    </button>
                </td>
            </tr>
            <tr class="ignoreRow">
                ...
                <td>{{bean.transCountP - (-bean.adjustModel.payableCount)}}</td>
                <td>{{bean.transAmtP/100 - (-bean.adjustModel.payableAmt) | currency:'￥'}}</td>
                <td>{{bean.transCountR - (-bean.adjustModel.receivableCount)}}</td>
                <td>{{bean.transAmtR/100 - (-bean.adjustModel.receivableAmt) | currency:'￥'}}</td>
                <td>{{((bean.transAmtP - bean.transAmtR)/100) - (-(bean.adjustModel.payableAmt -
                    bean.adjustModel.receivableAmt)) | currency:'￥'}}
                </td>
                <td>{{bean.feeAmtR/100 - (-bean.adjustModel.reFeeAmt) | currency:'￥'}}</td>
                <td>{{bean.feeAmtP/100 - (-bean.adjustModel.feeAmt) | currency:'￥'}}</td>
                <td>{{((bean.feeAmtR - bean.feeAmtP)/100) - (-(bean.adjustModel.reFeeAmt - bean.adjustModel.feeAmt)) |
                    currency:'￥'}}
                </td>
                <td colspan="8"></td>
            </tr>
