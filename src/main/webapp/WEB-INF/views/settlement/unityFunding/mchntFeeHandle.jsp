        ...
        <table table-detail>
            ...
            <tr class="ignoreRow">
                ...
                <td ng-if="vm.flag == '002'" style="padding: 0 0 0 0"><input type="number" style="width: 45px"
                                                                             ng-model="bean.adjustModel.payableCount">
                </td>
                <td ng-if="vm.flag == '002'" style="padding: 0 0 0 0"><input type="number" style="width: 45px"
                                                                             ng-model="bean.adjustModel.payableAmt">
                </td>
                <td ng-if="vm.flag == '002'" style="padding: 0 0 0 0"><input type="number" style="width: 45px"
                                                                             ng-model="bean.adjustModel.receivableCount"></td>
                <td ng-if="vm.flag == '002'" style="padding: 0 0 0 0"><input type="number" style="width: 45px"
                                                                             ng-model="bean.adjustModel.receivableAmt">
                </td>
                <td ng-if="vm.flag == '002'">{{bean.adjustModel.payableAmt - bean.adjustModel.receivableAmt | currency:''}}</td>
                <td ng-if="vm.flag == '003'" style="padding: 0 0 0 0"><input type="number" style="width: 45px"
                                                                             ng-model="bean.adjustModel.reFeeAmt">
                </td>
                <td ng-if="vm.flag == '003'" style="padding: 0 0 0 0"><input type="number" style="width: 45px" ng-model="bean.adjustModel.feeAmt">
                </td>
                <td ng-if="vm.flag == '003'">{{bean.adjustModel.reFeeAmt - bean.adjustModel.feeAmt | currency:''}}</td>
                <td colspan="7">备注：<input ng-model="bean.adjustModel.remark"></td>
                <td>
                    <button ng-show="vm.item.clrFlag == '0'" ng-disabled="commonModalForm.$invalid"
                            ng-click="vm.saveFeeAdjust(bean,bean.adjustModel)">保存
                    </button>
                </td>
            </tr>
            <tr class="ignoreRow">
                <th  colspan="4">调整后</th>
                <td ng-if="vm.flag == '002'">{{bean.payableCount - (-bean.adjustModel.payableCount)}}</td>
                <td ng-if="vm.flag == '002'">{{bean.payableAmt/100 - (-bean.adjustModel.payableAmt) | currency:'￥'}}</td>
                <td ng-if="vm.flag == '002'">{{bean.receivableCount - (-bean.adjustModel.receivableCount)}}</td>
                <td ng-if="vm.flag == '002'">{{bean.receivableAmt/100 - (-bean.adjustModel.receivableAmt) | currency:'￥'}}</td>
                <td ng-if="vm.flag == '002'">{{((bean.payableAmt - bean.receivableAmt)/100) - (-(bean.adjustModel.payableAmt -
                    bean.adjustModel.receivableAmt)) | currency:'￥'}}
                </td>
                <td ng-if="vm.flag == '003'">{{bean.feeAmt/100 - (-bean.adjustModel.reFeeAmt) | currency:'￥'}}</td>
                <td ng-if="vm.flag == '003'">{{bean.reFeeAmt/100 - (-bean.adjustModel.feeAmt) | currency:'￥'}}</td>
                <td ng-if="vm.flag == '003'">{{((bean.feeAmt - bean.reFeeAmt)/100) - (-(bean.adjustModel.reFeeAmt - bean.adjustModel.feeAmt)) |
                    currency:'￥'}}
                </td>
                <td colspan="8"></td>
            </tr>
