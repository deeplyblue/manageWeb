...
import com.oriental.clearDubbo.model.clear.mchnt.MchntClearDayModel;
...
@Slf4j
@Controller
@RequestMapping("settlement/clear/day/mchntClearDay")
public class MchntClearDayController {
    ...
    @RequestMapping("init")
    @RequiresPermissions("mchntClearDay_search")
    public String init() {
        return "settlement/clear/day/mchntClearDay";
    }

    @OperateLogger(content = "商户日结明细查询",operationType = OperateLogger.OperationType.R)
    @RequestMapping("search")
    @RequiresPermissions("mchntClearDay_search")
    @ResponseBody
    public ResponseDTO<Pagination<MchntClearDayModel>> queryPage(Pagination<MchntClearDayModel> pagination,
                                                                 OrgClearDayModel orgClearDayModel,
                                                                 String beginDate, String endDate) {
        ResponseDTO<Pagination<MchntClearDayModel>> responseDTO = new ResponseDTO<Pagination<MchntClearDayModel>>();
        try {
            ...
            log.info("商户日结明细查询参数："+requestModel);
            ResponseModel<MchntClearDayModel> responseModel = clearDayInterface.selectMchntClearDay(requestModel);
            log.info("商户日结明细查询结果："+responseModel);
    ...
    @RequiresPermissions("mchntClearDay_down")
    ...
    public ResponseEntity<byte[]> download(OrgClearDayModel orgClearDayModel,
        ...
        File tempFile = null;
        try {
            tempFile = fileDownAjax.touch("商户日结明细" + DateUtils.now() + ".xlsx");
            orgClearDayModel.setSettleDateBegin(DateUtils.parse(beginDate));
            orgClearDayModel.setSettleDateEnd(DateUtils.parse(endDate));
            RequestModel<OrgClearDayModel> requestModel = new RequestModel<OrgClearDayModel>();
            requestModel.setRequest(orgClearDayModel);
            ...
            tHeaders.add(new HeaderExt("商户代码", "mchntCode", ExcelContentExt.MCHNT));
            tHeaders.add(new HeaderExt("机构代码", "payOrgCode",ExcelContentExt.ORGANIZE));
            tHeaders.add(new HeaderExt("清算日期", "settleDate"));
            tHeaders.add(new HeaderExt("交易类型", "transCode",ExcelContentExt.MCHNT_TRANS_CODE));
            ...
            tHeaders.add(new HeaderExt("佣金(元)", "feeAmt",ExcelContentExt.CURRENCY));
            int i=1;
            if(responseModel.getResponseResult()){
                for(MchntClearDayModel mchntClearDayModel : responseModel.getList()){
                    Map map = Bean2MapUtil.convertBean(mchntClearDayModel);
                    map.put("index",i);
                    datas.add(map);
                    i++;
                }
            ...
            File file = ExcelUtils.addTitle(tempFile, "商户日结明细表", fileDownAjax, tHeaders.size(), beginDate, endDate);
