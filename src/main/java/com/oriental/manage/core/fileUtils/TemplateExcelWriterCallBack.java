 ...
 */
@Slf4j
public class TemplateExcelWriterCallBack implements ExcelWriterCallBack {

    @Getter
    private ExcelWriterConfig config;

    public TemplateExcelWriterCallBack(ExcelWriterConfig config) {
        this.config = config;
    }
    ...
    public void render(ExcelWriterProcessor processor) {
        try {
            ...
            if (config.getSheetName() != null)
                sheet = processor.start(config.getSheetName(), false);
            else {
                throw new BusiException("cannot get sheet name");
            }
    ...
    protected void prepareMerges() {
        ...
        for (String header : cols.keySet()) {
            ...
            if (config.getStartWith() > 0){
                countNumber = lastMergeNumber = config.getStartWith() - 1;
            }else {
    ...
    public void buildData() {
        ...
        if (config.getStartWith() != 0) {
            Row row = sheet.getRow(config.getStartWith() - 1);
            processor.setNowRow(row);
        }
        ...
        for (int x = 0; x < datas.size(); x++) {
            ...
            for (Header header : headers) {
                ...
                initCell(x, index, cell, header.getField(), formatContent(header, value));
                if (index++ == 0) {
    ...
    private Object formatContent(Header header, Object value) {
        ...
        if (headerExt.getExt() != null) {
            switch (headerExt.getExt()) {
                ...
                case    PAY_BANK_CODE:
                    if (StringUtils.isNullOrEmpty(Constants.getDataDictMap().get("PAY_BANK_CODE").get(value))) {
                        return value;
                    }
                    return Constants.getDataDictMap().get("PAY_BANK_CODE").get(value);
                case BUSI_TYPE:
                    if (StringUtils.isNullOrEmpty(Constants.getDataDictMap().get("MCHNT_BUSI_TYPE").get(value))) {
                        return value;
                    }
