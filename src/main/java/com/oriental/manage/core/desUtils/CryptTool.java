...
public class CryptTool {
	...
	public static SecretKey genDESKey() throws Exception {
		...
		String keyStr = constants.PAY_TRANS_CRYPT_KEY;
		return genDESKey(keyStr);
	}
    ...
    public static String des3Base64Enc(SecretKey key, String str)throws Exception {
        if(key==null)
            key = genDESKey();
        byte[] enc = desEncrypt(key,str.getBytes("UTF-8"));
	...
	public static void main(String[] args) {
		try {
			...
			log.info("encbase64="+encbase64);


            byte[] decbase64 = base64DecodeToBytes(encbase64);
			log.info("decbase64="+new String(decbase64,"UTF-8"));
